...
import { addComposer } from "./composer.ts";
...
// let handle = AddObject(scene, "tools/handle.glb", [0, -2, 0]);
 ...
 hammer.then((mesh) => { addToAnimLoop(mesh, 0.05); scene.add(mesh); addClickable(mesh); AddLabel("Hammer", mesh) })
...
const composer=addComposer(renderer,scene,camera,width,height)
